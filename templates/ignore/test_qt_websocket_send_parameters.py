...
class Client_WS(QtCore.QObject):

    CheckParameterResponseSignal = Signal(dict)

    def __init__(self, video_object, parent = None):
        super(self.__class__, self) .__init__(parent)
        self.video_object = video_object
        self.video_object.FrameSignal.connect(self.send_frame_qt)
        self.video_object.ParameterSignal.connect(self.send_random_paramaters_qt)
    ...
    def send_random_paramaters_qt(self):
        ...
        parameter_payload.xMinE = random.random()
        ...
        #Send custom Parameters
        #print(f'Parameter y_track_state is: {parameter_payload.y_track_state}')
        #print(f'Parameter autozoom_state is: {parameter_payload.autozoom_state}')
        #parameter_payload.gamma = 0.99
        self.CheckParameterResponseSignal.emit(parameter_payload.dict())
        self.ws.sendBinaryMessage(content)
...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        self.client_ws.CheckParameterResponseSignal.connect(self.sent_parameters)
        self.client_ws.ws.textMessageReceived.connect(self.print_tracking_results)
        self.client_ws.ws.textMessageReceived.connect(self.check_received_parameters)
    ...
    @Slot(str)
    def check_received_parameters(self, message):
        if self.parameters is not None:
            if (json.loads(message) == pytest.approx(self.parameters)):
                var = 'autozoom_state'
                recv_var = json.loads(message)[var]
                exp_var = self.parameters[var]
                print(f'Message Received: {recv_var}, expected is {exp_var}')
                print('Parameters are matching')
                pass
            else:
                raise AssertionError
            
        else:
            print("self.parameters is still None")

    @Slot(dict)
    def sent_parameters(self, parameter_payload):
        self.parameters = parameter_payload
...
class Video_Object(QtCore.QObject):
    ...
    def read_video(self):
        ...
        while video.isOpened():
            ...
            #Random sending the of parameters.
            i = random.randint(1,3)
            if i == 2:
                self.ParameterSignal.emit()
