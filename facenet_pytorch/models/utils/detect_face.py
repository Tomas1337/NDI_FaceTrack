...
@torch.jit.ignore
def is_tensor_check(tensor):
    return torch.is_tensor(tensor)

def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device, is_np_ndarray=False):
    if is_np_ndarray or is_tensor_check(imgs):
